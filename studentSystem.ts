...
class Student {
  ...
  showStatus() {
    ...
    console.log(
      chalk.cyan.bold(` > Selected Course     : ${this.selectedCourse}`)
    );
    console.log(
      chalk.cyan.bold(` > Balance             : ${this.balance} Pending`)
    );
  }
  async paidFees(roll: number) {
  ...
  removeStd(roll: number) {
    ...
    if (find === undefined) {
      ...
      console.log(
        `Name : ${find.name}  Roll No : ${find.rollNo} has removed Succesfully`
      );
    }
  }
  async addCourse(roll: number) {
    let find = students.find((s) => s.rollNo === roll);
...
while (true) {
  ...
  if (explore.action === "Exit") {
    ...
    console.log(
      chalk.yellowBright(
        ` ${action.name}, Roll Number ${data.rollNo} \n You select ${action.course} and its fees is ${fee} `
      )
    );
    students.push(data);
